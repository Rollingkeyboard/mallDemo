...
if(!empty($_POST['name'])){
    ...
    //入库处理
    unset($sql,$obj,$res);
    $sql="INSERT goods(name,price,des,content,pic,user_id,create_time,update_time,view) 
                values('{$name}','{$price}','{$des}','{$content}','{$pic}','{$userId}','{$_SERVER['REQUEST_TIME']}','{$_SERVER['REQUEST_TIME']}',0)";
    $obj=mysqli_query($con,$sql);
    if($obj){
