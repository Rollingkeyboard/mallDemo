...
if(!empty($_POST['username'])){
    ...
    unset($sql,$obj,$res);
    $sql="INSERT user(username,password,create_time) value('{$username}','{$password}','{$_SERVER['REQUEST_TIME']}')";
    $obj=mysqli_query($con,$sql);
    if($obj){
        $userId=mysqli_insert_id($con);
        msg(1,"恭喜注册成功，您的用户名为:{$username},用户id为:{$userId}",'login.php');
        exit();
    }else {
        msg(2,mysqli_error());
